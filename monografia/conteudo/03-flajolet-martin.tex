...
\begin{quote}
  \textbf{Contagem Distinta Aproximada:} dado um conjunto $\mathbb{M}$, \textbf{estimar} o número de elementos 
...
conjunto $\mathbb{M}$ para um número inteiro entre $0$ e $2^L-1$, em que $L$ é a quantidade necessária de bits para 
mapear todas as entradas de $\mathbb{M}$ (na prática, este valor é $32$ ou $64$). Assim, para cada $x_i \in \mathbb{M}$, 
...
Dessa forma, o algoritmo mantém a ocorrência dos prefixos de cada $\textit{hash}$ em um vetor $BITMAP[0 \dots L-1]$, de 
modo que $BITMAP[i] = 1$, se e somente se, o prefixo $0^i1$ apareceu entre os \textit{hashes} dos elementos de 
$\mathbb{M}$. 
...
$\mathbb{M}$ será $2^R/\phi$, em que, $\phi$ é um fator de correção que será discutido em 
...
O interesse passa a ser, portanto, estimar $\mathbb{E}[R_n]$ a partir dessa fórmula aproximada de $p_{n,k}$, de maneira 
que 
