...
Uma solução para esse problema é inserir cada elemento de $\mathbb{M}$ em uma tabela hash. 
...
Uma das primeiras soluções para a \textbf{contagem distinta aproximada} foi apresentada no artigo ~\citep{flajolet:martin:85}.
A motivação para o desenvolvimento desse algoritmo foi a otimização de pesquisas nos bancos de dados relacionais, 
em que a identificação do número de elementos distintos em uma coluna era a principal dificuldade no processo de otimização.