%!TeX root=../tese.tex
%("dica" para o editor de texto: este arquivo é parte de um documento maior)
% para saber mais: https://tex.stackexchange.com/q/78101/183146

% Os capítulos de compõem a dissertação/tese, com numeração normal, podem
% ser inseridos diretamente aqui ou "puxados" de outros arquivos.
% Em alguns (raros) casos, pode ser interessante usar \include ao
% invés de \input: https://tex.stackexchange.com/a/32058/183146

\input{conteudo/01-introducao.tex}
\par

\input{conteudo/02-morris.tex}
\par

\input{conteudo/03-flajolet-martin.tex}
\par

\input{conteudo/04-adaptive-sampling.tex}
\par

\input{conteudo/05-loglog.tex}
\par

\input{conteudo/06-aplicacoes.tex}
\par

\input{conteudo/07-conclusao.tex}